  ...
  const initializeWebSocket = (promptValue: string, chatBoxSettings: ChatBoxSettings) => {
    ...
    if (!socket && typeof window !== 'undefined') {
      ...
      newSocket.onmessage = (event) => {
        ...
        // If the backend sends a "finished" message, clear heartbeat and close the socket.
        if (data.type === "finished") {
          // Optionally process the termination content.
          console.log("Received termination message:", data.content);
      ...
      newSocket.onopen = () => {
        ...
        const startPayload = "start " + JSON.stringify({ task: promptValue, report_type, report_source, tone, headers });
        newSocket.send(startPayload);

        // Start heartbeat using newSocket
        heartbeatInterval.current = window.setInterval(() => {
          // Check before sending ping
